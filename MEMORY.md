...
- PR autonomy (2026-02-17): I may open/post PRs without asking first (still draft-first for any outbound comms).
- Deploy autonomy (2026-02-17): I may push live/deploy without asking **for low-risk changes** when there is an obvious rollback path (e.g., Vercel deploy/alias/rollback). Still escalate/ask first for high-risk or hard-to-reverse production changes (DB migrations, destructive data ops), secrets/security changes, billing/finance, legal-ish, cancellations/refunds/chargebacks, or angry/upset threads.
