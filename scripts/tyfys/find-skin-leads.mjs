#!/usr/bin/env node
/**
 * Find Zoho Leads that mention skin/eczema/dermatitis/scars in:
 * - related Notes
 * - related Attachments filenames
 *
 * Outputs a CSV + JSON in memory/ for outreach targeting.
 *
 * Usage:
 *   node scripts/tyfys/find-skin-leads.mjs --pages 20 --perPage 200
 *   node scripts/tyfys/find-skin-leads.mjs --days 365 --maxLeads 2000
 */
...
const days = Number(getArg('--days', '3650')); // default ~10y to approximate “entire list” but still bounded by pages/maxLeads
...
async function listLeadsPage({ accessToken, page }) {
  // Pull only fields we need for outreach.
  const fields = [
    'id',
    'Full_Name',
    'First_Name',
  ...
  // Zoho criteria expects: (Modified_Time:after:YYYY-MM-DD)
...
async function main() {
  ...
  for (let page = 1; page <= pages; page += 1) {
    ...
    for (const lead of leads) {
      ...
      for (const n of notes) {
        ...
        if (hits.length) {
          noteHits.push({ hits, snippet: String(body).slice(0, 160).replace(/\s+/g, ' ').trim() });
        }
      }
      ...
      // Also try a light match on the lead name/status if those contain keywords (rare but cheap)
      const leadFieldHits = [];
      const maybeText = [lead?.Lead_Status].filter(Boolean).join(' ');
      const lfHits = matchKeywords(maybeText);
      if (lfHits.length) leadFieldHits.push({ hits: lfHits, source: 'Lead_Status', text: maybeText });

      if (noteHits.length || attHits.length || leadFieldHits.length) {
        const allHits = new Set();
        for (const x of [...noteHits, ...attHits]) for (const h of x.hits) allHits.add(h);
        for (const x of leadFieldHits) for (const h of x.hits) allHits.add(h);
  ...
  for (const r of results) {
    const exampleNote = r.noteHits?.[0]?.snippet || '';
    const exampleAtt = r.attHits?.[0]?.fileName || '';
    lines.push([
      r.leadId,
      r.name,
      r.owner,
