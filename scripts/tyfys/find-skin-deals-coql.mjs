#!/usr/bin/env node
/**
 * Find Zoho Deals (clients) with skin/eczema/dermatitis/scars signals via
 * COQL over Notes + Attachments, then resolve Parent_Id -> Deals.
 *
 * Output:
 *  - memory/opendoor/skin-deals-<date>.csv
 *  - memory/opendoor/skin-deals-<date>.json
 */
...
const outBase = `skin-deals-${new Date().toISOString().slice(0, 10)}`;
